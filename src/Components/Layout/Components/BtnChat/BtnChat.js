...
function BtnChat() {
    ...
    const test = [
        {
            ...
            timestamp: '2024-07-12T15:38:22.630Z',
            userId: 2,
        },
        {
            content: 'Má',
        ...
        {
            content: 'Kệ con mẹ mày thằng chó',
            senderId: 2,
            sender_type: 'shop',
            timestamp: '2024-07-12T15:38:22.630Z',
            userId: 2,
    ...
    const finalTest = test.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
                            ...
                            <div className={cx('list_conversations')}>
                                <div className={cx('container_list_conversations')}>
                                    {mess_vs_shop.map((item) => {
                                        const shop = shops.filter((s) => s.idShop === item.idShop);
                                        if (shop.length != 0) {
                                            return (
                                                <div
                                                    className={cx(
                                                        'item_conversations',
                                                        idshop == shop[0].idShop ? 'active' : '',
                                                    )}
                                                    onClick={() => {
                                                        setidshop(shop[0].idShop);
                                                    }}
                                                >
                                                    <img src="https://media3.scdn.vn/img4/2021/02_27/SzznwDekJyRQDajzy6Nd.jpg"></img>
